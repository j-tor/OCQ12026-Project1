...
void MipsDisplay::ClearScreen(uint32_t color)
{
    std::fill(std::begin(vram), std::end(vram), color);
    Flush();
}

void MipsDisplay::ClearVRAM()
{
    std::fill(std::begin(vram), std::end(vram), 0);
}

void MipsDisplay::SetDisplayPixel(int x, int y, uint32_t color)
{
